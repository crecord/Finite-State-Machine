	...
	function checkTransition (name,currentEvent) { 
		...
		for (var i = 0; i< currentState.transitions.length; i++){
			...
			if (trans.element === undefined){
					eventElementMatch = (elements[0] === event.target);
				}
				else{
					eventElementMatch = (trans.element === event.target);
				}
			...
			if (trans.input === name & eventElementMatch ){
				...
				//console.log("I was listening to this element: " + event.target.id);
...
for (var i= (elements.length-1); i >= 0; i--){
  ...
  element.addEventListener("mouseout",function(event){
    ...
    console.log("I just Moused out of : "+ event.target.id);
    currentEvent=event;
    var name= "mouseOut"
  	checkTransition (name,currentEvent,element);
  });
  ...
  element.addEventListener("mouseover",function(event){
   console.log("I just Moused into: "+ event.target.id);
    currentEvent=event;
    var name= "mouseIn"
  	checkTransition (name,currentEvent, element);
  }); 
